...
export function DataDashboardTab({ theme = 'light' }: DataDashboardTabProps) {
  ...
  const [organizationId] = useState<string>('demo-org-1'); // Demo org ID
  ...
  const renderOverview = () => (
    <div className="space-y-6">
      ...
      <div className={cn(
        ...
        <div>
          <div className="font-medium">
            API Status: {healthLoading ? 'Checking...' : healthData?.success ? 'Healthy' : 'Error'}
          </div>
          {healthData && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard
          title="Total Revenue"
          value={metricsData?.data?.totalRevenue ? `$${(metricsData.data.totalRevenue / 1000).toFixed(0)}K` : '$0'}
          icon={DollarSign}
          color="bg-green-500"
        ...
        <StatCard
          ...
          value={metricsData?.data?.efficiency ? `${(metricsData.data.efficiency * 100).toFixed(0)}%` : 'N/A'}
          icon={Target}
          color="bg-orange-500"
              ...
              {projectsData?.data?.slice(0, 3).map((project, index) => (
                <div key={index} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium truncate max-w-xs">{project.name}</div>
                      <div className={cn(
                        'text-sm capitalize',
                        theme === 'dark' ? 'text-gray-400' : 'text-gray-500'
                      )}>
